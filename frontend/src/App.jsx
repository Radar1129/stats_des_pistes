...
const dicoCompagnies = {
  ...
  "BA": "BAW", "LH": "DLH", "SN": "BEL", "IB": "IBE", "VY": "VLG",
  "TK": "THY", "AT": "RAM", "TU": "TAR", "AH": "DAH", "3O": "MAC",
  "EI": "EIN", "LG": "LGL", "TB": "JAF", "EZY": "EZY"
};
...
function App() {
  ...
  const handleLogin = (e) => {
    ...
    .then(data => {
      ...
      } else setLoginError('❌ Identifiants ou mot de passe incorrects.');
    })
  ...
  const isBordeauxMovement = (avion) => {
    ...
    const volProgramme = volsPrevus.find(p => {
      ...
      const cleanP = p.vol.replace(/\s+/g, '').toLowerCase();
      const cleanC = avion.callsign.replace(/\s+/g, '').toLowerCase();
      const iata = cleanP.substring(0, 2).toUpperCase();
      const icao = dicoCompagnies[iata];
      const matchDico = icao && cleanC.toUpperCase().startsWith(icao);
      return cleanC.includes(cleanP) || cleanP.includes(cleanC) || matchDico || (cleanC.substring(0,3) === cleanP.substring(0,3));
    });

    const estArrivee = volProgramme && volProgramme.type?.toLowerCase().includes('arrivée');
    const estDepart  = volProgramme && volProgramme.type?.toLowerCase().includes('départ');
  ...
  const getPisteFromCap = (cap) => {
    ...
    if (cap >= 35 && cap <= 95) return "PISTE 05";
    if (cap >= 215 && cap <= 275) return "PISTE 23";
    if (cap > 95 && cap < 155) return "PISTE 11";
    if (cap > 275 && cap < 335) return "PISTE 29";
    return "LFBD";
  };
  ...
  const checkUrgence = (squawk) => {
    if (squawk === '7700') return "⚠️ URGENCE (7700)";
    if (squawk === '7600') return "🔇 PANNE RADIO (7600)";
    if (squawk === '7500') return "🚨 DÉTOURNEMENT (7500)";
    return null;
  };
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: '#f0f2f5' }}>
        <div style={{ backgroundColor: 'white', padding: '40px', borderRadius: '8px', boxShadow: '0 4px 15px rgba(0,0,0,0.1)', width: '100%', maxWidth: '400px', textAlign: 'center' }}>
          <h2 style={{ color: '#2c3e50', marginBottom: '25px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}><span style={{ fontSize: '1.2em' }}>🔒</span> Accès Radar</h2>
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            <input type="text" placeholder="Identifiant" value={usernameInput} onChange={(e) => setUsernameInput(e.target.value)} style={{ padding: '12px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '1em', outline: 'none' }} required />
            <input type="password" placeholder="Mot de passe" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} style={{ padding: '12px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '1em', outline: 'none' }} required />
            {loginError && <div style={{ color: '#e74c3c', fontSize: '0.9em', fontWeight: 'bold' }}>{loginError}</div>}
            <button type="submit" style={{ padding: '12px', backgroundColor: '#1a73e8', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '1em', fontWeight: 'bold' }}>Se connecter</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', maxWidth: '1400px', margin: '0 auto', textAlign: 'left', backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', paddingBottom: '10px', borderBottom: '1px solid #ddd' }}>
        ...
        <button onClick={handleLogout} style={{ padding: '6px 14px', backgroundColor: '#e74c3c', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.85em' }}>Déconnexion</button>
      </header>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#e74c3c', fontWeight: 'bold', fontSize: '0.95em' }}>
              🔔 <div style={{ display: 'flex', alignItems: 'center' }}>
                    <span style={{ textTransform: 'uppercase' }}>Alerte Mouvement Live (avec vérification sur carte Suivi Radar)</span>
                    <Tooltip content={`D'où viennent les données ?
Flux radar en temps réel (API adsb.lol / adsb.fi) limité à 15 km, croisé avec le programme officiel aspiré depuis le site de LFBD.

Comment sont-elles affichées ?
L'algorithme isole l'avion le plus pertinent via un plafond adaptatif (1500m en vol, 500m au sol). Il calcule la piste (QFU) via le cap boussole et synchronise son statut rouge sur la carte.`} />
                  </div>
            </div>
            {volLivePrincipal ? (
              <div style={{ marginTop: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
                  <span style={{ fontSize: '1.9em', fontWeight: 'bold', color: '#900' }}>Vol {volLivePrincipal.callsign}</span>
                  {volLivePrincipal.type && (
                    <span style={{ backgroundColor: '#fdecea', padding: '2px 6px', borderRadius: '4px', fontSize: '0.75em', fontWeight: 'bold', color: '#e74c3c' }}>{volLivePrincipal.type}</span>
                  )}
                </div>
                <div style={{ color: '#333', fontSize: '0.95em', marginTop: '6px', fontWeight: '500' }}>
                  ...
                  <div style={{ marginTop: '12px', display: 'flex', flexWrap: 'wrap', gap: '8px', fontSize: '0.9em' }}>
                    ...
                    {(volLivePrincipal.t || volLivePrincipal.desc) && (
                      <span style={{ padding: '4px 8px', background: '#e0e0e0', color: '#333', borderRadius: '4px', fontWeight: 'bold' }}>
                        {volLivePrincipal.t || volLivePrincipal.desc}
                      </span>
                    )}
                    {checkUrgence(volLivePrincipal.squawk) && (
                      <span style={{ padding: '4px 8px', background: '#d32f2f', color: '#fff', borderRadius: '4px', fontWeight: 'bold' }}>
                        {checkUrgence(volLivePrincipal.squawk)}
                      </span>
                    )}
                  </div>
              ...
              <div style={{ color: '#7f8c8d', fontSize: '1em', marginTop: '20px', fontStyle: 'italic', textAlign: 'center' }}>
                Aucun mouvement au sol ou à basse altitude à cet instant.
              </div>
            )}
          </div>
        ...
        <div style={{ backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '15px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              ...
              {totalAnnules > 0 && (
                <span style={{ fontSize: '0.75em', padding: '3px 8px', backgroundColor: '#fdecea', color: '#e74c3c', borderRadius: '4px', fontWeight: 'bold' }}>{totalAnnules} annulé(s)</span>
              )}
              <span style={{ fontSize: '0.75em', padding: '3px 8px', backgroundColor: '#e8f5e9', color: '#388e3c', borderRadius: '4px', fontWeight: 'bold' }}>{totalDetectes} / {totalPrevus} détectés ✓</span>
          ...
          {volsPrevus.length === 0 ? (
            ...
            <div ref={tableContainerRef} style={{ maxHeight: '300px', overflowY: 'auto', border: '1px solid #e2e8f0', borderRadius: '6px' }}>
              <table style={{ width: '100%', fontSize: '0.85em', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead style={{ position: 'sticky', top: 0, backgroundColor: '#fafafa', zIndex: 1, boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
                  <tr style={{ color: '#555', borderBottom: '1px solid #e2e8f0' }}>
                    <th style={{ padding: '6px 8px' }}>Heure</th>
                    <th style={{ padding: '6px 8px' }}>Type</th>
                    <th style={{ padding: '6px 8px' }}>Vol / Cie</th>
                    <th style={{ padding: '6px 8px' }}>Prov. / Dest.</th>
                    <th style={{ padding: '6px 8px', textAlign: 'right' }}>Statut</th>
                  </tr>
                </thead>
                <tbody>
                  {volsPrevus.map((v, i) => {
                    const statutVol = v.statut || v.remarque || '';
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                {vols.map((avion, index) => {
                  ...
                  return (
                    ...
                    >
                      <div>
                        <span style={{ fontSize: '1.05em', fontWeight: 'bold', color: couleurTexte }}>{avion.callsign}</span>
                        {avion.type && (
                          <span style={{ color: '#555', fontSize: '0.9em', marginLeft: '6px' }}>({avion.type})</span>
                        )}
                      </div>
          ...
          <h2 style={{ margin: 0, fontSize: '1.1em', fontWeight: 'bold', color: '#0f2042', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
            ...
            {historiqueStats.collecte_initiee && (
              <span style={{ fontSize: '0.75em', color: '#7f8c8d', fontWeight: 'normal', textTransform: 'none', marginLeft: '10px' }}>
                (Collecte initiée le {historiqueStats.collecte_initiee})
              </span>
            )}
          </h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1.9fr', gap: '40px' }}>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                {Object.entries(historiqueStats.axes).map(([axe, val]) => {
                  const pct = getSecurePct(val);
                  const volsCount = typeof val === "object" ? (val.vols ?? 0) : 0;
                  const isPrincipal = axe.toLowerCase().includes('principal');
                  const couleurBarre = isPrincipal ? '#1a73e8' : '#e74c3c';
                  return (
                    <div key={axe}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9em', fontWeight: 'bold', color: '#0f2042' }}>
                        <span>
                          {axe}{' '}
                          <span style={{ color: '#7f8c8d', fontSize: '0.9em', fontWeight: 'normal', marginLeft: '5px' }}>
                            ({volsCount} mouvement{volsCount > 1 ? 's' : ''})
                          </span>
                        </span>
                        <span style={{ fontSize: '1.1em' }}>{pct}%</span>
                      </div>
                      <div style={{ width: '100%', backgroundColor: '#eee', height: '8px', borderRadius: '4px', marginTop: '6px', overflow: 'hidden' }}>
                        <div style={{ width: `${pct}%`, backgroundColor: couleurBarre, height: '100%', borderRadius: '4px' }}></div>
                      </div>
                    </div>
                  );
                })}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                <div style={{ backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '12px', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                    {['Piste 05', 'Piste 23'].map((piste) => {
                      const data = historiqueStats.pistes[piste] || { pourcentage: 0, decollages: 0, atterrissages: 0 };
                      const pct = getSecurePct(data);
                      return (
                        <div key={piste} style={{ backgroundColor: '#f8f9fa', padding: '10px', borderRadius: '6px', textAlign: 'center' }}>
                          <div style={{ color: '#555', fontSize: '0.85em', fontWeight: 'bold' }}>{piste}</div>
                          <div style={{ fontSize: '1.4em', fontWeight: 'bold', margin: '6px 0', color: '#1a73e8' }}>{pct}%</div>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', fontSize: '0.75em', color: '#666', borderTop: '1px solid #eee', paddingTop: '6px', marginTop: '4px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Décollages:</span> <strong style={{color: '#333'}}>{data.decollages || 0}</strong></div>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Atterrissages:</span> <strong style={{color: '#333'}}>{data.atterrissages || 0}</strong></div>
                          </div>
                        </div>
                      );
                    })}
                ...
                <div style={{ backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '12px', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                    {['Piste 11', 'Piste 29'].map((piste) => {
                      const data = historiqueStats.pistes[piste] || { pourcentage: 0, decollages: 0, atterrissages: 0 };
                      const pct = getSecurePct(data);
                      return (
                        <div key={piste} style={{ backgroundColor: '#fdf2f2', padding: '10px', borderRadius: '6px', textAlign: 'center' }}>
                          <div style={{ color: '#555', fontSize: '0.85em', fontWeight: 'bold' }}>{piste}</div>
                          <div style={{ fontSize: '1.4em', fontWeight: 'bold', margin: '6px 0', color: '#e74c3c' }}>{pct}%</div>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', fontSize: '0.75em', color: '#666', borderTop: '1px solid #fee', paddingTop: '6px', marginTop: '4px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Décollages:</span> <strong style={{color: '#333'}}>{data.decollages || 0}</strong></div>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Atterrissages:</span> <strong style={{color: '#333'}}>{data.atterrissages || 0}</strong></div>
                          </div>
                        </div>
